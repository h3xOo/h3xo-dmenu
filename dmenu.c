...
static int (*fstrncmp)(char const *, char const *, size_t) = strncmp;
static char *(*fstrstr)(char const *, char const *) = strstr;

static unsigned int textw_clamp(char const *str, unsigned int n)
{
...
static char *cistrstr(char const *h, char const *n)
{
        size_t i = 0;
        ...
        for (token = strtok(tokens, " "); token; token = strtok(NULL, " ")) {
            ...
            // Move highlight str end, draw highlight, & restore
                        ...
                        if (indentx - (lrpad / 2) - 1 < maxw)
                                drw_text(drw, x + indentx - (lrpad / 2) - 1, y, MIN(maxw - indentx, TEXTW(highlight) - lrpad), bh, 0,
                                         highlight, 0);
...
static void insert(char const *str, ssize_t n)
{
...
static void vi_keypress(KeySym ksym, XKeyEvent const *ev)
{
        static size_t const quit_len = LENGTH(quit_keys);
 ...
 /* insertion */
  ...
  /* fallthrough */
        ...
        case XK_p:
                if (text[cursor] != '\0')
                        cursor = nextrune(+1);
                XConvertSelection(dpy, (ev->state & ControlMask) ? clip : XA_PRIMARY, utf8, utf8, win, CurrentTime);
                return;
        case XK_P:
                XConvertSelection(dpy, (ev->state & ControlMask) ? clip : XA_PRIMARY, utf8, utf8, win, CurrentTime);
                return;
 ...
 /* misc. */
        ...
        default:
                for (size_t i = 0; i < quit_len; ++i)
                        if (quit_keys[i].ksym == ksym && (quit_keys[i].state & ev->state) == quit_keys[i].state) {
                                cleanup();
                                exit(1);
                        }
        }
...
static void keypress(XKeyEvent *ev)
{
        ...
        if (using_vi_mode) {
                vi_keypress(ksym, ev);
                return;
        } else if (vi_mode && (ksym == global_esc.ksym && (ev->state & global_esc.state) == global_esc.state)) {
                using_vi_mode = 1;
                if (cursor)
                        cursor = nextrune(-1);
                goto draw;
...
static void buttonpress(XEvent *e)
{
     ...
     *       add that to the input width */
        if (ev->button == Button1 && ((lines <= 0 && ev->x >= 0 && ev->x <= x + w + ((!prev || !curr->left) ? TEXTW("<") : 0)) ||
...
static void paste(void)
{
    ...
    /* we have been given the current selection, now insert it into input */
        if (XGetWindowProperty(dpy, win, utf8, 0, (sizeof text / 4) + 1, False, utf8, &da, &di, &dl, &dl, (unsigned char **)&p) ==
                    Success &&
            p) {
...
static void setup(void)
{
    ...
    /* create menu window */
        ...
        win = XCreateWindow(dpy, root, x, y, mw, mh, border_width, CopyFromParent, CopyFromParent, CopyFromParent,
