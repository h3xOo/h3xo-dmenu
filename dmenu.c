...
static char *cistrstr(char const *h, char const *n)
{
        ...
        for (; *h; h++) {
                for (i = 0; n[i] && tolower((unsigned char)n[i]) == tolower((unsigned char)h[i]);
                     ++i)
                        ;
                if (n[i] == '\0')
                        return (char *)h;
...
static void buttonpress(XEvent *e)
{
        ...
        if (ev->button == Button2) {
                XConvertSelection(dpy, (ev->state & ShiftMask) ? clip : XA_PRIMARY, utf8, utf8, win,
                                  CurrentTime);
                drawmenu();
                return;
        }
...
static void paste(void)
{
        ...
        if (XGetWindowProperty(dpy, win, utf8, 0, (sizeof text / 4) + 1, False, utf8, &da, &di, &dl,
                               &dl, (unsigned char **)&p)
                == Success
            && p) {
...
static void setup(void)
{
        ...
        if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
                ...
                if (mon >= 0 && mon < n)
                        ...
                        if (XGetWindowAttributes(dpy, pw, &wa))
                                for (j = 0; j < n; j++)
                                        if ((a
                                             = INTERSECT(wa.x, wa.y, wa.width, wa.height, info[j]))
                                            > area) {
                                                area = a;
                                                i = j;
                                        }
        ...
        win = XCreateWindow(dpy, root, x, y, mw, mh, border_width, CopyFromParent, CopyFromParent,
                            CopyFromParent, CWOverrideRedirect | CWBackPixel | CWEventMask, &swa);
        if (border_width)
...
{
        die("usage: dmenu [-bcfiv] [-l lines] [-p prompt] [-bw border_width] [-fn font] [-m "
            "monitor]\n"
...
static void read_Xresources(void)
{
        ...
        if ((xrm = XResourceManagerString(drw->dpy))) {
                ...
                if (XrmGetResource(xdb, "dmenu.font", "*", &type, &xval)
                    == True) /* font or font set */
                        fonts[0] = strdup(xval.addr);
                if (XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval)
                    == True) /* normal background color */
                        colors[SchemeNorm][ColBg] = strdup(xval.addr);
                if (XrmGetResource(xdb, "dmenu.color4", "*", &type, &xval)
                    == True) /* normal foreground color */
                        colors[SchemeNorm][ColFg] = strdup(xval.addr);
                if (XrmGetResource(xdb, "dmenu.color4", "*", &type, &xval)
                    == True) /* selected background color */
                        colors[SchemeSel][ColBg] = strdup(xval.addr);
                if (XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval)
                    == True) /* selected foreground color */
