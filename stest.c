...
static void test(char const *, char const *);
...
static void test(char const *path, char const *name)
{
	struct stat st, ln;
	...
	if ((!stat(path, &st) &&
	     ...
	     && (!FLAG('h') || (!lstat(path, &ln) &&
				S_ISLNK(ln.st_mode))) /* symbolic link     */
	     &&
